    ...
    <x-slot name="header">
        <div class="flex">
            <h2 class="mt-2 w-full font-semibold text-xl text-gray-800 leading-tight pb-4">
                {{$account->name ? "$account->name ($account->number)" : "$account->number "}} {{ __('Transaction history') }}
            </h2>
                    ...
                    <div class="flex flex-row-reverse pb-4">
                        <form id="filter">
                            <div class="flex">
                                ...
                                <a href="/transactions/{{$account->id}}" class="ml-1 inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">Clear filtr</a>
                            </div>
                        </form>
                    </div>
